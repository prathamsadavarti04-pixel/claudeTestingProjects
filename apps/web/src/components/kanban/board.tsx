...
export function KanbanBoard({ workspaceId, canEdit }: { workspaceId: string; canEdit: boolean }) {
  ...
  const { data: tasks } = trpc.task.list.useQuery({ workspaceId }) as { data: Task[] | undefined };
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const targetStatus = overIsColumn ? (over.id as Task["status"]) : (tasks ?? []).find((t) => t.id === over.id)?.status;
